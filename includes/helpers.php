...
function assetUrl(string $path = ''): string
{
    ...
    //
    // مهر از زمان تغییر خود فایل ساخته می‌شود، نه از شماره نسخه سیستم:
    // هر فایلی که آپلود شود بلافاصله آدرس تازه می‌گیرد — حتی قالبی که
    // خود کاربر ویرایش کرده و شماره نسخه‌ای ندارد.
    if (preg_match('/\.(css|js)$/i', $path)) {
        $url .= (str_contains($url, '?') ? '&' : '?')
              . ltrim(assetStamp($path), '?');
    }
